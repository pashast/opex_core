<?php
namespace Opencart\Admin\Controller\Extension\Rex\Other;
class Rexcore extends \Opencart\System\Engine\Controller {

	public function index(): void {
		$this->load->language('extension/rex/other/rexcore');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/rex/other/rexcore', 'user_token=' . $this->session->data['user_token'])
		];

		$data['save'] = $this->url->link('extension/rex/other/rexcore.save', 'user_token=' . $this->session->data['user_token']);
		$data['back'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=other');

		//Regular fields
		//Default values
		$regular_fields = [
			'other_rexcore_status'          => 1,
			'other_rexcore_ac_limit'        => 5,
			'other_rexcore_ac_full_limit'   => 300,
			'other_rexcore_check_post_size' => 65534,
			'other_rexcore_mv_custom_class' => 'mb-4',
			'other_rexcore_lazy_script'     => 0,
			'other_rexcore_mobile_detect'   => 0,
		];
		...
		$this->response->setOutput($this->load->view('extension/rex/other/rexcore', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/rex/other/rexcore')) {
		...
		if (!$json) {
			...
			$this->model_setting_setting->editSetting('other_rexcore', $this->request->post);
	...
	public function reverseAutocompleteModules($var): array {
		$this->load->model('extension/rex/other/rexcore');
		$response = [];

		foreach ($var as $module_id) {
			$module_info = $this->model_extension_rex_other_rexcore->getModule($module_id);
			if ($module_info) {
				$response[] = [
	...
	public function reverseAutocompleteLayouts($var): array {
		...
		foreach ($var as $layout_id) {
			$layout_info = $this->model_extension_rex_other_rexcore->getLayout($layout_id);
			if ($layout_info) {
				$response[] = [
	...
	public function reverseAutocompleteAttributes($var): array {
		...
		foreach ($var as $attribute_id) {
			$attribute_info = $this->model_extension_rex_other_rexcore->getAttribute($attribute_id);
			if ($attribute_info) {
				$response[] = [
	...
	public function autocompleteInformation(): void {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_extension_rex_other_rexcore->getAutocompleteInformations($filter_data);
	...
	public function autocompleteModule(): void {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_extension_rex_other_rexcore->getAutocompleteModules($filter_data);
	...
	public function autocompleteLayout(): void {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_extension_rex_other_rexcore->getAutocompleteLayout($filter_data);
	...
	public function install(): void {
		$data = [
			'code'        => 'RexAddLazyModulesScript',
			'description' => 'Add lazy loading modules script to footer',
			'trigger'     => 'catalog/view/common/footer/after',
			'action'      => 'extension/rex/other/rexcore.eventAddLazyModulesScript',
			'status'      => true,
			'sort_order'  => 0
		];
		...
		$startup_data = [
			'code'        => 'rexcore_catalog',
			'description' => 'Startup rex core',
			'action'      => 'catalog/extension/rex/startup/rexcore',
			'status'      => 1,
			'sort_order'  => 1
		];
	...
	public function uninstall(): void {
		...
		$this->model_setting_event->deleteEventByCode('RexAddLazyModulesScript');
		$this->model_setting_startup->deleteStartupByCode('rexcore_catalog');
	}
}